...
#include <Arduino.h>                      //Chnaged to<> for frp, "" Platform IO
#include "SX128XLT_Definitions.h"         //Chnaged to "" from <> for Platform IO
...
class SX128XLT  {
  ...
  public:
    ...
    SX128XLT(); //Nothing to do when object is created

    //begin overload otions
    ...
    bool begin(int8_t pinNSS, int8_t pinNRESET, int8_t pinRFBUSY, int8_t pinDIO1, uint8_t device); //Used in main.cpp
    ...
    bool setupRanging(uint32_t frequency, int32_t offset, uint8_t modParam1, uint8_t modParam2, uint8_t  modParam3, uint32_t address, uint8_t role); //Used in main.cpp
    bool transmitRanging(uint32_t address, uint16_t timeout, int8_t txpower, uint8_t wait);//Used in main.cpp
